...
n_th = (n_e/9);    % non-specific matrix thalamus neurons
...
for t = 1:(sim_time-1)
    ...
    g_syn_tot = g_syn_AMPA + min(Mg_gate.*g_syn_NMDA,NMDA_max) + g_syn_GABA + g_syn_adapt + I_ext_AMPA + min(Mg_gate.*I_ext_NMDA,NMDA_max) + eps; % total condunctance
    E_tot = (g_syn_AMPA.*E_exc + min(Mg_gate.*g_syn_NMDA.*E_exc,NMDA_max) + g_syn_GABA.*E_inh + g_syn_adapt.*E_adapt + I_ext_AMPA.*E_exc + min(Mg_gate.*I_ext_NMDA.*E_exc,NMDA_max))./g_syn_tot; % total reverse potential
...
SmoothGausstwindow = @(x,n) conv(x,gausswin(n)./sum(gausswin(n)),'same');
...
rateL_smoothed = SmoothGausstwindow(rateL_count,250);
rateR_smoothed = SmoothGausstwindow(rateR_count,250);
...
LDistCrit_smoothed = SmoothGausstwindow(LDistCrit,250);
RDistCrit_smoothed = SmoothGausstwindow(RDistCrit,250);
...
BuRegimeL_smoothed = SmoothGausstwindow(BuRegimeL,250);
BuRegimeR_smoothed = SmoothGausstwindow(BuRegimeR,250);

% smooth adaptation
I_adapt_L = downsample(mean(I_adapt_store(1:45,:)),1/DT);
I_adapt_R = downsample(mean(I_adapt_store(46:90,:)),1/DT);
I_adapt_L_smoothed = SmoothGausstwindow(I_adapt_L,250);
I_adapt_R_smoothed = SmoothGausstwindow(I_adapt_R,250);
...
I_coupling_L_smoothed = SmoothGausstwindow(I_coupling_L,250);
I_coupling_R_smoothed = SmoothGausstwindow(I_coupling_R,250);
...
for idx = 1:length(rate_switchpoints)
    rateL_switchlocked(idx,:) = rateL_smoothed(rate_switchpoints(idx)-twindow:rate_switchpoints(idx)+twindow);
    rateR_switchlocked(idx,:) = rateR_smoothed(rate_switchpoints(idx)-twindow:rate_switchpoints(idx)+twindow);
end
...
for idx = 1:length(rate_switchpoints)
    critL_switchlocked(idx,:) = LDistCrit_smoothed(rate_switchpoints(idx)-twindow:rate_switchpoints(idx)+twindow);
    critR_switchlocked(idx,:) = RDistCrit_smoothed(rate_switchpoints(idx)-twindow:rate_switchpoints(idx)+twindow);
end
% stack L2R and R2L
...
% grab time series around rate switch points
for idx = 1:length(rate_switchpoints)
    BuL_switchlocked(idx,:) = BuRegimeL_smoothed(rate_switchpoints(idx)-twindow:rate_switchpoints(idx)+twindow);
    BuR_switchlocked(idx,:) = BuRegimeR_smoothed(rate_switchpoints(idx)-twindow:rate_switchpoints(idx)+twindow);
end
% stack L2R and R2L
...
for idx = 1:length(rate_switchpoints)
    I_adapt_L_switchlocked(idx,:) = I_adapt_L_smoothed(rate_switchpoints(idx)-adapt_twindow:rate_switchpoints(idx)+adapt_twindow);
    I_adapt_R_switchlocked(idx,:) = I_adapt_R_smoothed(rate_switchpoints(idx)-adapt_twindow:rate_switchpoints(idx)+adapt_twindow);
end
...
% ---------- coupling
for idx = 1:length(rate_switchpoints)
    I_coupling_L_switchlocked(idx,:) = I_coupling_L_smoothed(rate_switchpoints(idx)-adapt_twindow:rate_switchpoints(idx)+adapt_twindow);
    I_coupling_R_switchlocked(idx,:) = I_coupling_R_smoothed(rate_switchpoints(idx)-adapt_twindow:rate_switchpoints(idx)+adapt_twindow);
end
...
time = -twindow:1:twindow;
adapt_time = -adapt_twindow:1:adapt_twindow;
