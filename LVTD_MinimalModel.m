...
% max conductance enting soma (values above this are clipped)
g_max = 500;
...
for t = 1:(sim_time-1)
    ...
    % conductance
    Mg_gate = min((((v_d+80)./60).^2)./(1 + (((v_d+80)./60).^2)),Mg_max);
    ...
    % conductance
    Mg_gate = min((((v+80)./60).^2)./(1 + (((v+80)./60).^2)),Mg_max);
    g_syn_tot = g_syn_AMPA + Mg_gate.*g_syn_NMDA + g_syn_GABA + g_syn_adapt + W_FF.*I_ext_AMPA + W_FF.*Mg_gate.*I_ext_NMDA + eps; % total condunctance
    E_tot = (g_syn_AMPA.*E_exc + Mg_gate.*g_syn_NMDA.*E_exc + g_syn_GABA.*E_inh + g_syn_adapt.*E_adapt + W_FF.*I_ext_AMPA.*E_exc + W_FF.*Mg_gate.*I_ext_NMDA.*E_exc)./g_syn_tot; % total reverse potential
    I_syn  = g_syn_AMPA.*(E_exc - v) + Mg_gate.*g_syn_NMDA.*(E_exc - v) + g_syn_GABA.*(E_inh - v) + g_syn_adapt.*(E_adapt - v);
    ...
    v = (v + DV)./(1 + min(g_syn_tot,g_max).*(DT./C));
    u = u + DU;
