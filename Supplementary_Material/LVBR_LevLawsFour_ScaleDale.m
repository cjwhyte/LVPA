...
stimulus_strength = 1500:100:1800;
if simulation == 1
    % -- Levelt's fourth law
    counter = 0;
    for sim = 1:num_sims
        ...
        for stim_idx = 1:length(stimulus_strength)
            ...
            fr = stimulus_strength(stim_idx);
            stimulus = BR_Stimulus_ScaleDale(DT,params,fr,fr,0,0,0); % 0 = no lesion
            [Levs_FourthLaw_firings{stim_idx,sim}, ... 
             ~, ~, ~, ~] = ThalamoCorticalNetSimulator(DT,sim_time,params,stimulus);
        end 
    end 
    % save('Levs_FourthLaw_firings_ScaleDale','Levs_FourthLaw_firings','-v7.3')
elseif simulation == 0
    load('Levs_FourthLaw_firings_ScaleDale')
    params = ThalamoCorticalParams_ScaleDale(DT,sim_time);
end 
...
firings = Levs_FourthLaw_firings{1,1};
firings = firings(firings(:,2)<=params.n_e,:);

figure(1)
hold on
title('Raster: ttL5')
plot(firings(:,1)/10000,firings(:,2),'.', 'Color',[76, 109, 172]/255);
...
f = figure(3);
ax = gca;
errorbar(stimulus_strength,alternations_mean*3,alternations_err*3,'k-', 'LineWidth',4)
ax.FontSize = 20;
ax.LineWidth = 1.5;
set(gca, 'FontName', 'Times')
