function params = ThalamoCorticalParams_ScaleDale(DT,sim_time)

    params = {};

    %----- number of neurons

    params.n_e = 1600;              % excitatory LV cortical neurons
    params.n_i = 400;               % inhibitory LV cortical interneurons
    params.n_th = params.n_e/10;    % non-specific matrix thalamus neurons
    params.n_total = params.n_e + params.n_i + params.n_th;
    
    %----- connectivity
    
    % E/I ring
    params.e_e = 2.4; params.e_i = 1.5; params.i_e = 5.85;
    sd_e_e = .25; sd_e_i = 2; sd_i_e = 2;
    J_e_e = zeros(params.n_e,params.n_e); 
    J_e_i = zeros(params.n_i,params.n_e); 
    J_i_e = zeros(params.n_e,params.n_i);

    % Cortical ring coupling 
    sparsity = 0.1; % (imposed sparsity = 10%)
    theta_e = linspace(0,2*pi,params.n_e);  % location on ring
    theta_i = linspace(0,2*pi,params.n_i);  % location on ring
    for j = 1:params.n_e
        for k = 1:params.n_e
            % euclidean distance in coordinates of unit circle
            dist = sqrt(((cos(theta_e(j)) - cos(theta_e(k)))^2 + (sin(theta_e(j)) - sin(theta_e(k)))^2));
            J_e_e(j,k) = params.e_e*exp(-.5*(dist./sd_e_e).^2)./(sd_e_e*sqrt(2*pi))*(rand<=sparsity);
        end
    end
    ...
    for j = 1:params.n_e
        for k = 1:params.n_i
            % euclidean distance in coordinates of unit circle
            dist = sqrt(((cos(theta_e(j)) - cos(theta_i(k)))^2 + (sin(theta_e(j)) - sin(theta_i(k)))^2));
            J_e_i(k,j) = params.e_i*exp(-.5*(dist./sd_e_i).^2)./(sd_e_i*sqrt(2*pi))*(rand<=sparsity);
            J_i_e(j,k) = params.i_e*exp(-.5*(dist./sd_i_e).^2)./(sd_i_e*sqrt(2*pi))*(rand<=sparsity);
        end
    end
    ...
    pocket_size = params.n_e/params.n_th; counter = 1;
    for j = 1:params.n_th
    ...
    % adaptation current
    params.tau_decay_adapt = 2000; params.delta_adapt = 0.05;
